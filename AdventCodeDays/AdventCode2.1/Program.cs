...
int greenLimit = 13;
int blueLimit = 14;
string input = @"Game 1: 1 blue, 1 red; 10 red; 8 red, 1 blue, 1 green; 1 green, 5 blue
...
// bool possibleTest = IsPossible(testGame);
int sum = 0;
...
int GetGameId(string game){ 
    int gameId = int.Parse(game.Substring(game.IndexOf(' '), game.IndexOf(':') - 4));
    return gameId;
}
...
bool IsPossible(string game){
    ...
    game = game.Remove(0, game.IndexOf(':') + 1);
    rounds = game.Split(';');
    foreach(string round in rounds){
        ...
        foreach(string block in blocks){
            ...
            if(block.Contains("red")){
                redList.Add(int.Parse(block.Replace(" red", "")));
            }
            if(block.Contains("green")){
                greenList.Add(int.Parse(block.Replace(" green", "")));
            }
            if(block.Contains("blue")){
                blueList.Add(int.Parse(block.Replace(" blue", "")));
            }
        }
    }
    foreach(int value in redList){
        if(value > redLimit){
            return false;
        }
    }
    foreach(int value in blueList){
        if(value > blueLimit){
            return false;
        }
    }
    foreach(int value in greenList){
        if(value > greenLimit){
            return false;
        }
    }
